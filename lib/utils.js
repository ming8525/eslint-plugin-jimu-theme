...
function getPropsMapping(props) {
  ...
  props.forEach((prop, idx) => {
    if (prop.type === 'identifier' || prop.identifier === 'theme' || AllowedPart1Props.includes(prop.identifier)) {
      if (AllowedPrefixProps.includes(prop.identifier)) {
        mappings.push(prop)
      } else {
        if (prop.identifier === 'colors' && prevIdentifier === 'theme') {

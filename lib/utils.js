...
function getCSSUtilitiesMappingInNewTheme(prefix, colorName, colorShade) {
  if(!prefix || !colorName) return
  const prefixObj = colorUtilitiesMap[prefix]
  const colorObj = prefixObj && prefixObj[colorName]
  const utility = colorObj && colorObj[colorShade || 'default']
  return utility
}

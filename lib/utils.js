...
function isTargetFromSource(context, target, sources = []) {
  ...
  if (targetDefinition) {
    const memberExpressionNode = targetDefinition.defs[0].node && targetDefinition.defs[0].node.init
    const nodeNames = getLastTwoNodesName(memberExpressionNode)
    let isFromSource = false
    if(sources.length === 2) {
      isFromSource = nodeNames[0] === sources[0] && nodeNames[1] === sources[1]
    } else if (sources.length === 1) {
      isFromSource = nodeNames[0] === sources[0]
    }
    return isFromSource
  }
}
...
function isSpecifiedMemberExpressionNodeFromThemeProps(node, identifiers, sourceProps = {}) {
  ...
  if(match && sourceProps.source) {
    ...
    const fromSource = isTargetFromSource(context, target, [source]) || isTargetFromSource(context, target, ['props', source])
    return  fromSource
  }
  return match
}
...
function getMemberExpressionFromChainExpression(node) {
...
module.exports = {
  ...
  getMemberExpressionFromChainExpression,

...
const AllowedPrefixProps = ['this', 'props', 'theme', 'theme2']
const AllowedThemeProps = ['theme', 'theme2']
const AllowedPart1Props = ['colors', 'typography', 'link']
...
function matchNodeIdentifiers(node, identifiers = [], variables = [], allowedPrefixProps = AllowedPrefixProps, longest = true) {
...
function getPropsMapping(props) {
  ...
  props.forEach((prop, idx) => {
    if (prop.type === 'identifier' || AllowedThemeProps.includes(prop.identifier) || AllowedPart1Props.includes(prop.identifier)) {
      if (AllowedPrefixProps.includes(prop.identifier)) {
        mappings.push(prop)
      } else {
        if (prop.identifier === 'colors' && AllowedThemeProps.includes(prevIdentifier)) {
        ...
        } else if (prop.identifier === 'border' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getBorderPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'borderRadiuses' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getBorderRadiusPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'boxShadows' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getShadowPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'sizes' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getSizesPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'gutters' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getGuttersPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'surfaces' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getSurfacePropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'typography' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getTypographyMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'focusedStyles' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getFocusedStylesPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'header' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getHeaderFooterPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'footer' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getHeaderFooterPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'body' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getBodyPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'link' && AllowedThemeProps.includes(prevIdentifier)) {
          const { type, mappings: _mappings } = getLinkPropsMapping(props.slice(idx))
          mappings = type === 'string' ? _mappings : mappings.concat(_mappings)
        } else if (prop.identifier === 'darkTheme' && AllowedThemeProps.includes(prevIdentifier)) {

...
const VariableMap = {
  ...
  colorOptional: 'theme?.sys.color.divider.primary',
  width: '1px'
}
...
function reportMessages(node, context, sourceCode) {
  ...
  if (varName === 'color') {
    ...
    const otional2 = isSpecifiedObjectOptinal(node, 'border')
  ...
  create(context) {
    return {
      MemberExpression(node) {
        ...
        if (isSpecifiedMemberExpressionNode(node, ['theme', 'border', ['type', 'color', 'width']])) {
          reportMessages(node, context, sourceCode)
        }
        if (isSpecifiedMemberExpressionNode(node, ['border', ['type', 'color', 'width']])) {
          reportMessages(node, context, sourceCode)
        }
      }
    }

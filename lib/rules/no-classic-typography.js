'use strict'
const { DocURL, isSpecifiedMemberExpressionNode, getMemberExpressionNodeOptinalObject, TypographyVariantsMap, TypographyStylesMap, TypographyColorsMap, TypographyWeightsMap, TypographyLineHeightsMap } = require('../utils')
...
function getTypographyNameAndScopeInNewTheme(part1, part2, part3) {
  ...
  if (part1 === 'sizes') {
    ...
    if (part3 === 'color') {
      ...
      varName = ''
      surfix = TypographyColorMap[part2.slice(0, -1)]
    } else {
      varName = TypographyVariantsMap[part2]
    }
  }
...
function reportMessages(node, context, scope, fiveLevel = false) {
  const otional = getMemberExpressionNodeOptinalObject(node, ['theme', scope])
  const part1 = fiveLevel ? node.object.object.property.name : node.object.property.name
  const part2 = fiveLevel ? node.object.property.name : node.property.name
  const part3 = fiveLevel ? node.property.name : null
  const fixedText = getTypographyVariableMappingInNewTheme(part1, part2, part3, otional)
  ...
  create(context) {
    return {
      MemberExpression(node) {
        if (isSpecifiedMemberExpressionNode(node, ['theme', 'typography', 'variants', Object.keys(TypographyVariantsMap), Object.keys(TypographyStylesMap)])) {
          reportMessages(node, context, 'variants', true)
        } else  if (isSpecifiedMemberExpressionNode(node, ['theme', 'typography', 'variants', Object.keys(TypographyVariantsMap)])) {
          reportMessages(node, context, 'variants')
        } else if (isSpecifiedMemberExpressionNode(node, ['theme', 'typography', 'sizes', Object.keys(TypographyVariantsMap)])) {

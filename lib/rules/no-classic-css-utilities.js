'use strict'
const { getColorCSSUtilitiesMappingInNewTheme, getOthersCSSUtilitiesMappingInNewTheme } = require('../utils')


function reportMessagesForColorsUtilities(node, match, removeDeprecated, context) {
  ...
  const newUtility = getColorCSSUtilitiesMappingInNewTheme(prefix, colorName, colorShade)
  if (!newUtility) {
    let fix
    if (removeDeprecated) {
...
function reportMessagesForOtherUtilities(node, match, removeDeprecated, context) {
  const text = node.value.value
  const oldUtility = match[0]
  const start = node.value.range[0] + match.index + 1
  const end = start + match[0].length
  const newUtility = getOthersCSSUtilitiesMappingInNewTheme(oldUtility)
  if (!newUtility) {
    let fix
    if (removeDeprecated) {
      fix = fixer => fixer.replaceTextRange([start - (match.index === 0 ? 0 : 1), end + (match.index === 0 ? (node.value.value.includes(' ') ? 1 : 0) : 0)], '')
      ...
      JSXAttribute(node) {
        if (node.name.name === 'className' && node.value && node.value.type === 'Literal' && typeof node.value.value === 'string') {
          ...
          while ((match = regex1.exec(text)) !== null) {
            reportMessagesForColorsUtilities(node, match, removeDeprecated, context)
          }
          while ((match = regex2.exec(text)) !== null) {
            reportMessagesForColorsUtilities(node, match, removeDeprecated, context)
          }
          while ((match = regex3.exec(text)) !== null) {
            reportMessagesForOtherUtilities(node, match, removeDeprecated, context)
          }

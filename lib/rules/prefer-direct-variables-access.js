...
function reportNoPropsMessages(node, context, usedProps) {
  const sourceCode = context.sourceCode
  const rawCode = sourceCode.getText(node)
  const fixedText = usedProps[0] === 'this' ? rawCode.replace(/this\??\.props\??\./, '') : rawCode.replace(/props\??\./, '')
  context.report({
    node: node,
      ...
      MemberExpression(node) {
        ...
        if (((res = isSpecifiedScopeMemberExpressionNode(node, ['colors', [...ThemeAllColors, 'transparent']], { context, target: 'colors', source: 'theme', prefixProps: ['props', 'theme'] })).match)) {
          if (!res.usedProps) {
            ...
            reportNoPropsMessages(node, context, res.usedProps)
          }
